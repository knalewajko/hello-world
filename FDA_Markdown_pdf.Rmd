...
In my analysis I will use EU Decision-making data base (DEU) II data set [@thomson2012] as source for the response variable. Predictor variables will include independent and control variables necessary for the estimation of aforementioned hypotheses. 
...
![An example controversy from the legislative proposal on the labelling of spirit drinks: What should vodka be made of and how should it be labelled?](Figure1.png)
...
description <- c('Unique IDs of negotiated issues (chr)','Negotiation year (num: 1995-2009)','Country code (chr)','Initial position of each country on a given issue (num: 0-100)','Salience score of each issue for the given country (num: 0-100)','Position of the European Commission on a given issue (num: 0-100)','Position of the European Parliament on a given issue (num: 0-100)', 'Final outcome of the given negotiation (num: 0-100)','Absolute distance between member state and European Commission on each issue (num: 0-100)','Absolute distance between member state and European Parliament on each issue (num: 0-100)','Measure of negotiation success = ||initial position - negotiation outcome|-100| (num: 0-100)','Saliency-weighted success score (num: 0-100)','Natural log of the saliency-weighted success score (num: 0-4.6)','Dummy for eurozone membership (log: TRUE/FALSE)','Dummy for holding Council presidency (log: TRUE/FALSE)','Dummy for new member state of the EU (log: TRUE/FALSE)','Dummy for net contributor to EU budget (log: TRUE/FALSE)','Voting power in the European Council (num: 0.94-11.16)','Population size (num: 0.4-86 M)','Distance on orientation towards EU integration between member state and the Council (CHES) (num: 0-3)','Log of the EU integration distance (num: 0-1.36)','Distance on left-right ideological position scale between member state and the Council (CHES) (num: 0-2.5)')
...
In order to make the data set suitable for analysing the mentioned hypotheses, I "cleaned" my data in three steps.
...
```{r, echo=FALSE, fig.height=4, fig.width=3, warning=FALSE, two_plots_same_size_side_by_side}
# Load tidy data
...
plot1 <- country_success %>% 
  ...
  labs(x = "Unweighted bargaining success",
       ...
       title = "Mean negotiation success",
...
plot2 <- country_success2 %>% 
  ...
  labs(x = "Saliency-weighted bargaining success",
       y = "EU member state",
       color = "New member state",
       title = "Saliency-weighted success",
       caption = "Source: DEU II (1995-2009).") +
  theme_bw() +
  theme(legend.position = "bottom", 
        legend.direction = "horizontal",
...
plot3 <- ggplot(data=subset(db, !is.na(presidency)), aes(x = presidency, y = salsucc)) + 
  ...
  labs(x = "Council presidency holder at the time of negotiation",
       ...
       title = "Saliency-weighted success per presidency holder",
       subtitle = "Data points are transformed distances between outcomes and positions",
       caption = "Source: DEU II (1995-2009).") +
  theme_bw()
plot3
```
...
```{r, fig.height=2, fig.width=4, fig.align='center', echo=TRUE, warning=FALSE}
...
```{r, fig.height=3, fig.width=4, fig.align='center', echo=TRUE, warning=FALSE}
# MODELS 1 & 2
...
[^6]: For the country fixed-effects models please refer to the html version of this paper. The models in question could not have been included here due to the big number of independent variables (additional 26 countries).
[^7]: For all conducted tests please refer to sub-chapter Regression diagnostics in section *VIII. Annex*.
[^8]: For stargazer output including all models please refer to sub-chapter Complete regression in section *VIII. Annex* of the html version of this paper.